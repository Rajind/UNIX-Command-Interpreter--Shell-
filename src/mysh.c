...
void handleUserCommand()
{
	if(strstr(buffer, "&&") != NULL){
		parseCommand();
		while(commandConsecutive < commandC){
			populateMultipleCommand(command[commandConsecutive]);
...
void launchJob(char *command[], char *file, int newDescriptor,
               int executionMode)
{
	logMessage("enter launch job");
        pid_t pid;
        pid = fork();
        switch (pid) {
        case -1:
                perror("MY-shell(fork)");
                exit(EXIT_FAILURE);
                break;
        case 0:
                /**
                ...
                if (executionMode == FOREGROUND)
                        tcsetpgrp(MYSH_TERMINAL, getpid());                     // if we want the process to be in foreground
...
void putJobForeground(t_job* job, int continueJob)
{
        ...
        tcsetpgrp(MYSH_TERMINAL, job->pgid);                                 // give it the control of the terminal
        ...
        tcsetpgrp(MYSH_TERMINAL, MYSH_PGID);                                 // give the shell control of the terminal
}
...
void putJobBackground(t_job* job, int continueJob)
{
        ...
        tcsetpgrp(MYSH_TERMINAL, MYSH_PGID);                    // paranoia: give the shell control of terminal
}
...
void init()
{
        MYSH_PID = getpid();                                                // retrieve the pid of the shell
        MYSH_TERMINAL = STDIN_FILENO;                                	    // terminal = STDIN
        MYSH_IS_INTERACTIVE = isatty(MYSH_TERMINAL);                        // the shell is interactive if STDIN is the terminal

        if (MYSH_IS_INTERACTIVE) {                                          // is the shell interactive?
                while (tcgetpgrp(MYSH_TERMINAL) != (MYSH_PGID = getpgrp()))
                        kill(MYSH_PID, SIGTTIN);                            // make sure we are in the foreground
                ...
                setpgid(MYSH_PID, MYSH_PID);                                 // we make the shell process as new process group leader
                MYSH_PGID = getpgrp();
                if (MYSH_PID != MYSH_PGID) {
                        printf("Error, the shell is not process group leader");
                        exit(EXIT_FAILURE);
                }
                if (tcsetpgrp(MYSH_TERMINAL, MYSH_PGID) == -1)              // if bdsh cannot grab control of the terminal
                        tcgetattr(MYSH_TERMINAL, &MYSH_TMODES);             // we save default terminal attributes for shell.

                currentDirectory = (char*) calloc(1024, sizeof(char));
        } else {
                printf("Could not make MY-shell interactive. Exiting..\n");
                exit(EXIT_FAILURE);
        }
}
